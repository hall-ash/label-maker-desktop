...
const AboutModal = ({ isOpen, toggle }) => {
  return (
    <Modal isOpen={isOpen} toggle={toggle} centered scrollable size="lg" keyboard>
      ...
      <ModalBody>
        ...
        <p>
          You can adjust font size, text alignment, etc. by clicking the gear icon.
          Label text is automatically resized to fit the label dimensions.
        </p>
        ...
        <p>
          ...
          in the <strong>Start On Label</strong> input field will print labels starting on column 2, row 2
          of the sheet. Leaving it blank will start printing on the first label coordinate (A1).
        </p>
...
2: B3, D11, A17`}
        ...
        <p>Each page of skipped labels should be listed on a separate line.</p>

        <h4 className="mt-4">Add Aliquots</h4>
        <p>
          <strong>Add Aliquots</strong> generates labels that append
          <em> "1 of n" </em> to the end of the label text, where <em>n</em> 
          represents the number of replicates of that aliquot.
        </p>

        <h4 className="mt-4">Calculate Aliquots</h4>
        <p>
          If you have a material with a known concentration and volume and need to create aliquots of
          specific masses (e.g., 50 mg, 100 mg, 200 mg), enter the concentration and total volume, then
          specify the desired aliquot amounts (in any order) in the <strong>Aliquot Amounts</strong> input field
          (e.g., <code>[200, 50, 100]</code>). You can add commas between amounts or leave them out like this:
          <code>[50 100 200]</code>.
        </p>
        <p>The volumes for each aliquot will be calculated and labels generated accordingly.</p>
      </ModalBody>
